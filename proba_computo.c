...
static int proba_matmat_f(void)
{
    ...
    pfr_in_gpu_mitte_f(a); pfr_in_gpu_mitte_f(b); pfr_in_gpu_mitte_f(c);
    pfr_matmat_f(c, a, b);
    pfr_ex_gpu_cape_f(c);

    int ok = 1;
    for (int i = 0; i < 9; i++)
        if (!fere_aequalis_f(c->data[i], b->data[i])) { ok = 0; break; }

    pfr_matrix_destrue_f(a); pfr_matrix_destrue_f(b); pfr_matrix_destrue_f(c);
...
static int proba_matvec_f(void)
{
    ...
    pfr_in_gpu_mitte_f(a); pfr_in_gpu_mitte_vf(x); pfr_in_gpu_mitte_vf(y);
    pfr_matvec_f(y, a, x);
    ...
    pfr_matrix_destrue_f(a); pfr_vector_destrue_f(x); pfr_vector_destrue_f(y);
...
static int proba_dotum_f(void)
{
    ...
    pfr_in_gpu_mitte_vf(x); pfr_in_gpu_mitte_vf(y);

    float res = 0.0f;
    pfr_dotum_f(&res, x, y);

    pfr_vector_destrue_f(x); pfr_vector_destrue_f(y);
...
static int proba_matmat_magna_f(void)
{
    ...
    pfr_in_gpu_mitte_f(a); pfr_in_gpu_mitte_f(c);
    pfr_matmat_f(c, a, a);
    pfr_ex_gpu_cape_f(c);
    ...
    pfr_matrix_destrue_f(a); pfr_matrix_destrue_f(c);
...
static int proba_matmat_d(void)
{
    ...
    a->data[0] = 1; a->data[4] = 1; a->data[8] = 1;
    for (int i = 0; i < 9; i++) b->data[i] = (double)(i + 1);

    pfr_in_gpu_mitte_d(a); pfr_in_gpu_mitte_d(b); pfr_in_gpu_mitte_d(c);
    pfr_matmat_d(c, a, b);
    pfr_ex_gpu_cape_d(c);

    int ok = 1;
    for (int i = 0; i < 9; i++)
        if (!fere_aequalis_d(c->data[i], b->data[i])) { ok = 0; break; }

    pfr_matrix_destrue_d(a); pfr_matrix_destrue_d(b); pfr_matrix_destrue_d(c);
...
static int proba_matvec_d(void)
{
    ...
    pfr_in_gpu_mitte_d(a); pfr_in_gpu_mitte_vd(x); pfr_in_gpu_mitte_vd(y);
    pfr_matvec_d(y, a, x);
    ...
    pfr_matrix_destrue_d(a); pfr_vector_destrue_d(x); pfr_vector_destrue_d(y);
...
static int proba_dotum_d(void)
{
    ...
    x->data[0] = 1; x->data[1] = 2; x->data[2] = 3;
    y->data[0] = 4; y->data[1] = 5; y->data[2] = 6;
    pfr_in_gpu_mitte_vd(x); pfr_in_gpu_mitte_vd(y);

    double res = 0.0;
    pfr_dotum_d(&res, x, y);

    pfr_vector_destrue_d(x); pfr_vector_destrue_d(y);
...
static int proba_matmat_magna_d(void)
{
    ...
    a->data[0] = 2; a->data[1] = 1;
    a->data[2] = 1; a->data[3] = 2;
    pfr_in_gpu_mitte_d(a); pfr_in_gpu_mitte_d(c);
    pfr_matmat_d(c, a, a);
    pfr_ex_gpu_cape_d(c);
    ...
    pfr_matrix_destrue_d(a); pfr_matrix_destrue_d(c);
