...
static void circulum_pinge(
    uint32_t *alveus, int lat, int alt,
...
int main(void)
{
    ...
    while (currit) {
        ...
        if (celer < 1)
            celer = 1;
        if (claves[PFR_SC_SINISTRUM] || claves[PFR_SC_A])
            cx -= celer;
        if (claves[PFR_SC_DEXTRUM]   || claves[PFR_SC_D])
            cx += celer;
        if (claves[PFR_SC_SURSUM]    || claves[PFR_SC_W])
            cy -= celer;
        if (claves[PFR_SC_DEORSUM]   || claves[PFR_SC_S])
            cy += celer;
        ...
        circulum_pinge(
            imago, LATITUDO, ALTITUDO, cx, cy, radius,
