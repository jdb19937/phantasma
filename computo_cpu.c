                      ...
                      const pfr_vector_f_t *w, float eps)
{
    if (!o || !x || !w) return -1;
    int n = x->n;
    if (o->n != n || w->n != n) return -1;
    float ss = 0.0f;
    for (int j = 0; j < n; j++) ss += x->data[j] * x->data[j];
    ss = 1.0f / sqrtf(ss / n + eps);
    for (int j = 0; j < n; j++) o->data[j] = w->data[j] * ss * x->data[j];
    return 0;
}
...
int pfr_cpu_softmax_f(pfr_vector_f_t *x)
{
    ...
    int n = x->n;
    float mx = x->data[0];
    for (int i = 1; i < n; i++) if (x->data[i] > mx) mx = x->data[i];
    float s = 0.0f;
    for (int i = 0; i < n; i++) { x->data[i] = expf(x->data[i] - mx); s += x->data[i]; }
    for (int i = 0; i < n; i++) x->data[i] /= s;
    return 0;
}
...
{
    if (!o || !q || !cache_k || !cache_v || !att) return -1;
    if (d <= 0 || n_capita <= 0 || n_capita_kv <= 0) return -1;
    int hd      = d / n_capita;
    ...
    for (int h = 0; h < n_capita; h++) {
        ...
        for (int t = 0; t <= positio; t++) {
            ...
            float a = att_h[t];
            for (int i = 0; i < hd; i++) o_h[i] += a * v_t[i];
        }
    }
    return 0;
}
