...
{
    (void)p; (void)forma; (void)accessus;
                         ...
                         const void *pixels, int passus)
{
    ...
    if (rect) {
        ...
        for (int y = 0; y < rect->alt; y++)
            memcpy(t->pixels + (rect->y + y) * t->latitudo + rect->x,
                   fons + y * passus,
                   (size_t)rect->lat * 4);
    } else {
        if (passus == t->latitudo * 4)
            memcpy(t->pixels, pixels,
                   (size_t)t->latitudo * t->altitudo * 4);
        else {
            const unsigned char *fons = (const unsigned char *)pixels;
            for (int y = 0; y < t->altitudo; y++)
                memcpy(t->pixels + y * t->latitudo,
                       fons + y * passus,
                       (size_t)t->latitudo * 4);
        }
    }
...
{
    if (!p) return -1;
    uint32_t color = ((uint32_t)p->color_a << 24)
...
{
    if (!p) return -1;
    if (x < 0 || x >= p->latitudo || y < 0 || y >= p->altitudo) return 0;
...
int pfr_rectum_imple(pfr_pictor_t *p, const pfr_rectum_t *rect)
{
    ...
    if (x1 > p->latitudo) x1 = p->latitudo;
    if (y1 > p->altitudo) y1 = p->altitudo;
    for (int y = y0; y < y1; y++)
        for (int x = x0; x < x1; x++)
...
{
    if (numerus) *numerus = PFR_SC_NUMERUS;
    return ph.claves;
}
